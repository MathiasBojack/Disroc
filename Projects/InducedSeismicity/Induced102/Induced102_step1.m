...
Sx_load  = -20e6;   % The applied normal stress is considered positive when the direction is aligned with the outer normal
Sy_load  = -40e6;
...
[~, sortedIndex] = sort(prob_info.coordinate(nodes_topBC,1));
nodes_top_ranked = nodes_topBC(sortedIndex);

nodes_rightBC      = nodeList(abs(prob_info.coordinate(:,1)-5000)<eps);
[~, sortedIndex] = sort(prob_info.coordinate(nodes_rightBC,2));
nodes_right_ranked = nodes_rightBC(sortedIndex);

boundary.Sn.count = length( nodes_topBC) + length( nodes_rightBC) -2;

for i=1:(length(nodes_top_ranked)-1)
     boundary.Sn.text{i} = [ num2str(nodes_top_ranked(i+1)) ',' ...
         num2str(nodes_top_ranked(i)) ',' num2str(Sy_load) ', 0.0, 0.0'];
end

for i=1:(length(nodes_right_ranked)-1)
%     if ~(ismember(nodes_rightBC(i),prob_info.connectivity.joint) && ...
%             ismember(nodes_rightBC(i+1),prob_info.connectivity.joint) )
     boundary.Sn.text{i+length( nodes_topBC) -1} = ...
         [ num2str(nodes_right_ranked(i)) ',' num2str(nodes_right_ranked(i+1))...
         ',' num2str(Sx_load) ', 0.0, 0.0'];
%     end
end
...
cmd_txt = ['if exist ',foldername,'\RepH.dat del' ...
    foldername,'\RepH.dat'];
system(cmd_txt);
...
xlabel('$\tau / \sigma _n$','interpreter','latex')
ylabel('$Y$','interpreter','latex')

figure(5);clf;hold on;
xlabel('$U_t$','interpreter','latex')
ylabel('$Y$','interpreter','latex')

step_list = 1:numStepCalculated;
for i = length(step_list):length(step_list)
