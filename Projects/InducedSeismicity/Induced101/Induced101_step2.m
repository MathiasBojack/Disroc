...
% % Imposed displacement to see the evolution of shear stress
% boundary.Ux.count = 2*length(joint_upper);
% for i=1:length(joint_upper)
%     % lower disp fixed
%     boundary.Ux.text{i} = [ num2str(joint_lower(i)) ', ' num2str(0) ', 0.0, 0.0'];
%     % upper disp imposed
%     boundary.Ux.text{i+length(joint_upper)} = [ num2str(joint_upper(i)) ', ' num2str(1e-1) ', 0.0, 0.0'];
% end
...
boundary.Tau.count = prob_info.numJointElem;

for i = 1 : prob_info.numJointElem
    boundary.Tau.text{i} = [num2str(joint_nodes_L(i)) ', ' num2str(joint_nodes_K(i)) ...
