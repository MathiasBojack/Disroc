...
Imposed_disp = 0; % The applied shear stress is considered positve when Ut is positive.
resumption.active = 1;
resumption.step.active=1;
resumption.step.number = 2;
saveas_step.number = 302;
...
% % Adding a slightly more shear displacement on a single segment to see the evolution
boundary.Ux.count = length(joint_lower)+2;
for i=1:length(joint_lower)
    % lower disp fixed
    boundary.Ux.text{i} = [ num2str(joint_lower(i)) ', ' num2str(0) ', 0.0, 0.0'];
end
boundary.Ux.text{length(joint_lower)+1} = ...
          [ num2str(joint_nodes_L(node_elem_applied_stress)) ', ' num2str(Imposed_disp) ', 0.0, 0.0'];
boundary.Ux.text{length(joint_lower)+2} = ...
          [ num2str(joint_nodes_K(node_elem_applied_stress)) ', ' num2str(Imposed_disp) ', 0.0, 0.0'];
...
Parameter.calpara.tolerance.displacement= 1e-7;
...
for i = 1:length(step_list)
    ...
    figure(3);plot(faultplot.Tau/1e6 ,faultplot.X)
end
